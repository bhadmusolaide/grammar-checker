...
async function debugCareerService() {
  try {
    console.log('Testing career service job decoder...');
...
- Strong problem-solving skills and attention to detail`;
    ...
    // Test with no model config (should default to Groq)
    console.log('Testing with default Groq model...');
    const response1 = await runJobDecoder(jobDescription);
    console.log('Groq response received:', response1.substring(0, 200) + '...');
    
  } catch (error) {
    console.error('Error in career service:', error.message);
    if (error.response) {
